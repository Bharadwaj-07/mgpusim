...
run_benchmark() {
  ...
  (
    cd "$config" || exit 1
    echo "Starting benchmark: $benchmark at $(TZ='India/Kolkata' date)" >&2
...
while [ $scheduled_benchmarks -lt $total_benchmarks ]; do
  ...
  # Calculate number of benchmarks to run based on RAM (7GB per benchmark)
  num_benchmarks=$((available_ram / 7))

  # Cap at maximum 7 benchmarks, minimum 1
  if [ $num_benchmarks -gt 7 ]; then
    num_benchmarks=7
